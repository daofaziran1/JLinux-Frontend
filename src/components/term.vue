...
export default {
    ...
    return {
      ...
      commandList: [
        ...
        },
        {
          cmd: "grep",
          format:
            "grep [-bcdFGHilLnov][-A<显示行数>][-B<显示列数>][-C<显示列数>][-d<进行动作>] [范本样式][文件或目录...]",
          params: [
  ...
  methods: {
    ...
    uploadSuccess(response, file, fileList) {
      ...
      this.term.write(
        this.strHandle(
          this.textColor.brightWhite + this.userInfo.user,
          this.userInfo.path
        )
      );
    },
    ...
    login() {
      ...
      axios
        ...
        .then((res) => {
          ...
          this.term.write(
            this.textColor.brightWhite +
              this.strHandle(this.userInfo.user, this.userInfo.path)
          );
          // 自适应屏幕大小
          this.fitAddon.fit();
        ...
        */
        ...
        if (code === 13) {
          ...
          if (cmds[0] == "") {
            ...
            // help 无其他参数，格式 help
            if (cmds.length === 1) {
              this.commandList.forEach((res) => {
                this.term.writeln(
                  this.textColor.yellow + res.cmd + ":" + res.desc
                );
              ...
              if (cmds[1] == "list") {
                ...
                for (let i = 0; i < len; ++i) {
                  ...
                  this.term.write(
                    this.textColor.yellow + this.commandList[i].cmd + "\t"
                  );
                  if (i == len - 1) {
                    this.term.write("\n");
                  }
            ...
            if (cmds.length == 1) {
              ...
              axios({
                ...
                if (res.data.msg) {
                  ...
                  this.term.writeln(
                    this.textColor.green + "文件" + res.data.name + "下载完成。"
                  );
                  // 输出新的一行
                  this.term.write(
                    this.strHandle(
            ...
            axios
              ...
              .then((res) => {
                ...
                if (command === "pwd") {
                  this.term.write(
                    "\x1b[1;33m" + this.strHandle(res.data.res[0])
                  );
                } else if (command === "ls") {
                  // 遍历文件
                  ...
                  if (res.data.msg) {
                    res.data.res.forEach((v) => {
                      this.term.writeln(
                        "\t" + this.textColor.brightMagenta + v
                      );
                    });
                  }
                } else if (command === "cat") {
